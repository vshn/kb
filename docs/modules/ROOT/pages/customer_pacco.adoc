...
This is a special how-to for customers which want do do puppet module / profile development. The idea is to need access to as few as possible puppet modules in our gitlab, meaning that the VM will not get most of the VSHN basic puppet modules, mainly just the customer profile / the profile that wants to be developed on and it's dependencies.

There are of course things like not purging the vagrant user, packagemanger, etc. which we cannot live without. 
...
--
+
You can clone this to any directory you want. The folder does not need to be called "pacco".

. Create a SSH RSA key WITHOUT a passphrase, and add it in GitLab to your user. Keep this key safe and personal.
** *Do not share the created puppet master VM*, as this key is copied into that VM on provision.
** Do not use your normal SSH key with passphrase removed. You need to be able to remove this key from GitLab at anytime. 
...
--
+
This will take a while. Red messages in vagrant output are kind of normal – errors are not, though. 
...
+
*Do not forget to set the correct environment*, the one from step 8.
+
[source,bash]
...
* If you get such an error, just re-run provision / puppet. This is a cache issue inside puppet, we did not yet find a fix for this:
