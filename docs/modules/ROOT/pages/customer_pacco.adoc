...
This is a special how-to for customers which want do do puppet module / profile development. The idea is to need access to as few as possible puppet modules in our gitlab, meaning that the VM won't get most of the VSHN basic puppet modules, mainly just the customer profile / the profile that wants to be developed on and it's dependencies.

There are of course things like not purging the vagrant user, packagemanger, etc. which we can't live without.
